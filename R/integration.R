...
FindIntegrationAnchorsParallel <- function(
  object.list = NULL,
  assay = NULL,
  reference = NULL,
  anchor.features = 2000,
  ...
  if (reduction == "rlsi" || reduction == "cca") {
    stop("cca and rlsi is not supported in FindIntegrationAnchorsParallel")
  }
  if (normalization.method == "SCT") {
    stop("SCT is unsupported")
  }
  object.ncells <- sapply(X = object.list, FUN = function(x) dim(x = x)[2])
  if (any(object.ncells <= max(dims))) {
    bad.obs <- which(x = object.ncells <= max(dims))
  ...
  # determine all anchors
  anchoring.fxn <- function(row, combinations, object.list, 
  ...
  if (!require(doParallel)) {
    ...
    all.anchors <- tryCatch(
      expr = {
        cluster <- makeCluster(cores, # number of cores to use
                                    type = "PSOCK") # type of cluster
        registerDoParallel(cluster)
        all.anchors <- foreach(i=seq_len(nrow(combinations)),
